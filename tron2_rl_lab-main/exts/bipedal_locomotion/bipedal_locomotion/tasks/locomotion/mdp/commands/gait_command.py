...
class GaitCommand(CommandTerm):
    ...
    def __init__(self, cfg: UniformGaitCommandCfg, env: ManagerBasedEnv):
        ...
        # command format: [frequency, phase offset, contact duration, swing height]
        ...
        self.stand_still_flag = torch.zeros(self.num_envs, device=self.device)
    ...
    def _update_command(self):
        ...
        self.stand_still_flag = self._env.command_manager.get_term("base_velocity").is_standing_env # type: ignore

        # for standing env, set the gait index to the 0 (or phi_stance)
        standing_env_ids = self.stand_still_flag.nonzero(as_tuple=False).flatten()
        self.gait_indices[standing_env_ids] = 0

        # for walking env, update the gait index
        self.gait_indices = torch.remainder(
            self.gait_indices + self._env.step_dt * self.gait_command[:, 0] * ~self.stand_still_flag, 1.0)
